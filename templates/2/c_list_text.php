    ...
    <div class="container">
      <div class="row">
        <div class="col-lg-9 col-sm-8 blog-post-contents">
          ...
          foreach ($res as $val) {
            echo '<div class="blog-post"><h3><a href="'.d_url($val['id'],$val['d_link']).'">'.$val['d_name'].'</a></h3><div class="post-materials clearfix"><ul><li><h6><a href="#"><i class="fa fa-calendar"></i>'.local_date('M d Y',$val['d_date']).'</a></h6></li><li><h6><a href="#"><i class="fa fa-user"></i>'.(!empty($val['d_author'])?$val['d_author']:'网站管理员').'</a></h6></li><li><h6><a href="#"><i class="fa fa-tags"></i>'.(!empty($val['d_source'])?$val['d_source']:$cms['s_name']).'</a></h6></li></ul></div>'.($val['d_ifpicture']?'<div class="blog-image marginb30 margint30"><img alt="Blog Image" class="img-responsive" src="'.$val['d_picture'].'" ></div>':'').'<div class="post-content margint10"><p>'.str_cut(str_text($val['d_content']),100).'</p></div><div class="button mini-button button-style-2 margint30"><h6><a href="'.d_url($val['id'],$val['d_link']).'">READ MORE</a></h6></div></div>';
          }
          ?>
          <div class="pagination-container reset-clearfix">
            <hr>
            <ul class="pagination">
