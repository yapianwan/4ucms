...
if ($act == 'adminLogin') {
  ...
  $a_password = md5(str_safe($_POST['a_password']));
  
  // 次数限制
  times_limit('loginCount', 'time_admin');

  if ($res=$db->getRow("SELECT id,u_name FROM cms_user WHERE u_name='$a_name' AND u_psw='$a_password' AND u_isadmin=1")) {
